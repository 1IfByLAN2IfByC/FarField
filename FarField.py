...
class FarField:
 	...
 	def cart2rad(self, coord):
 		...
 		r = ( coord[:,0] ** 2 + coord[:,1] ** 2 + coord[:,2] ** 2)** .5
 	...
 	def expandGrid(self, degIncrement):
 		...
 		coordinates = concatenate((xd, yd, zd), axis=1)
 		phi, th, r = self.cart2rad(coordinates)
 		...
 		# elif abs(th[1] - th[0]) > tol && abs(phi[1] - phi[0]) < tol:
 		# 	while abs(phi[ii+1] - phi[1]) < tol:
 		# 		ii = ii+1
 		...
 		# # rotate the section about the axial axis
 		# th_mapped = zeros((Nth*2, Nphi*nRotations))
 		# th_mapped[0:Nth, 1:2*Nphi] = hstack(th_reg, th_reg)

 		# ph_mapped = zeros((Nth*2, Nphi*nRotations))
 		# ph_mapped[0:Nth, 1:2*Nphi] = hstack(phi_reg, (2*degIncrement - phi_reg[::-1])) 
 		# # ^includes the mirror symmetry plane

 		# R = zeros((Nth*2, Nphi*nRotations))
 		# R = r(1)




 		# return ph_mapped, th_mapped, R
...
# plt.ylabel('Y-axis')
# # plt.show()
>>>>>>> 8aada3d337c42bbbd90cbafce3b5f1a0e0d83b8e
